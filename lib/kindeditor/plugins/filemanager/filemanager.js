		...
		function createView(result) {
			...
			for (var i = 0, len = fileList.length; i < len; i++) {
				...
				div.mouseover(function(e) {//滑过这个div的时候
				    ...
				    K(this).children().eq(2).css("display","block");//如果是图片格式就查找当前下面的第3个标签元素设置为可见
				})
				.mouseout(function(e) {//划出这个循环div时
				    K(this).children().eq(0).removeClass('ke-on');//删除ke-on类
				    K(this).children().eq(2).css("display","none");//如果是图片格式设置当前下第3个标签为隐藏
				});
			...
			$(".xl_span").click(function(){//这里的.xl_span对应着刚才插入的删除按钮上的class
			    ...
			    $.post('http://ning.img.wxbuluo2.com/file_manager_json.php',{action:"delete",url:$this.attr("data-url")},function(res){//jquery的post，action为指定的配合后端用，url是获取刚才存在删除按钮上的图片路径，你完全可以用别的来写，因为用不好kindeditor的js库才用的jquery
